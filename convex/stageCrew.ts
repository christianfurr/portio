...
export const get = query({
  args: {},
  handler: async (ctx) => {
    return await ctx.db.query("stageCrew").first();
  },
});
...
export const upsert = mutation({
  args: {
    ...
    bio: v.string(),
    roles: v.array(
      v.object({ role: v.string(), description: v.string() })
    ),
    shows: v.array(
      v.object({ title: v.string(), role: v.string() })
    ),
    equipment: v.array(
      v.object({ name: v.string(), category: v.string() })
    ),
    software: v.array(
      v.object({ name: v.string(), proficiency: v.string() })
    ),
